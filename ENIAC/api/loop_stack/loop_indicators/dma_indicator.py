    ...
    def next(self):
        if self.cross[0] == 1:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.goldencross[0] = compare(self.dma.dma[0] - self.dma.ama[0], self.logic) and self.dma.dma[
                    0] > 0
...
class iDmaCrossDie(iBaseIndicator):
    ...
    def next(self):
        if self.cross[0] == -1:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.diecross[0] = compare(self.dma.ama[0] - self.dma.dma[0], self.logic) and self.dma.dma[0] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.diecross[0] = compare(self.dma.ama[0] - self.dma.dma[0], self.logic) and self.dma.dma[0] < 0
            else:
...
class iDmaCrossLong(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(dmas) == self.args[2]:
            dmalong = set(
                list(map(lambda d: d > 0, dmas + amas)) + [dmas[i] > amas[i] for i in range(len(dmas))] + list(
                    map(lambda s: sorted(s) == s, [dmas, amas])))
        else:
            dmalong = []

        if len(dmalong) == 1 and True in dmalong:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.dmalong[0] = True and self.dma.dma[0] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.dmalong[0] = True and self.dma.dma[0] < 0
            else:
...
class iDmaCrossShort(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(dmas) == self.args[2]:
            dmashort = set(
                list(map(lambda d: d < 0, dmas + amas)) + [dmas[i] < amas[i] for i in range(len(dmas))] + list(
                    map(lambda s: sorted(s, reverse=True) == s, [dmas, amas])))
        else:
            dmashort = []

        if len(dmas) != 0 and len(dmashort) == 1 and True in dmashort:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.dmashort[0] = True and self.dma.dma[0] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.dmashort[0] = True and self.dma.dma[0] < 0
            else:
...
class iDmaFall(iBaseIndicator):
    ...
    def next(self):
        ...
        dmas = list(self.dma.dma.get(size=self.args[2]))
        amas = list(self.dma.ama.get(size=self.args[2]))
        dmafall = set([dmas[i] < amas[i] for i in range(len(dmas))] + list(
            map(lambda s: sorted(s, reverse=True) == s, [dmas, amas])))
...
class iDmaTop(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(_list[dma_line]) == self.args[2] and _signal[dma_line] == max(_list[dma_line]):
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.dtop[0] = True and max(_list[dma_line]) > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.dtop[0] = True and max(_list[dma_line]) < 0
            else:
...
class iDmaBottom(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(_list[dma_line]) == self.args[2] and _signal[dma_line] == min(_list[dma_line]):
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.dbottom[0] = True and max(_list[dma_line]) > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.dbottom[0] = True and max(_list[dma_line]) < 0
            else:
