...
class iMacdCrossGolden(iBaseIndicator):
    ...
    def __init__(self):
        ...
        self.macd = btind.MACD(self.data.close, period_me1=self.args[0], period_me2=self.args[1],
    ...
    def next(self):
        ...
        if self.cross[0] == 1:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.goldencross[0] = compare(self.macd.macd[0] - self.macd.signal[0], self.logic) and \
                                            self.macd.macd[0] > 0
        ...
        # print(self.macd.macd[0], self.macd.signal[0], self.data.close[0], '*', self.data.datetime.date(),self.cross[0],self.lines.goldencross[0] )
...
class iMacdCrossDie(iBaseIndicator):
    ...
    def next(self):
        if self.cross[0] == -1:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.diecross[0] = compare(self.macd.signal[0] - self.macd.macd[0], self.logic) and \
                                         self.macd.macd[0] > 0
...
class iMacdLong(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(macdlong) == 1 and True in macdlong:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.macdlong[0] = True and self.macd.macd[0] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.macdlong[0] = True and self.macd.macd[0] < 0
            else:  # 其他条件
...
class iMacdShort(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(macdshort) == 1 and True in macdshort:
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.macdshort[0] = True and self.macd.macd[0] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.macdshort[0] = True and self.macd.macd[0] < 0
            else:  # 其他条件
...
class iMacdTop(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(_list) == self.args[3] and _list[-1] == max(_list):
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.macdtop[0] = True and _list[-1] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.macdtop[0] = True and _list[-1] < 0
            else:
...
class iMacdBottom(iBaseIndicator):
    ...
    def next(self):
        ...
        if len(_list) == self.args[3] and _list[-1] == min(_list):
            if self.logic["position"] == 1:  # 出现在上部  > 0
                self.lines.macdbottom[0] = True and _list[-1] > 0
            elif self.logic["position"] == -1:  # 出现在下部  < 0
                self.lines.macdbottom[0] = True and _list[-1] < 0
            else:
