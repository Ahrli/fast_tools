...
class SpotAPI(Client):
    ...
    def order_algo(self,instrument_id='',mode='',order_type='',size='',side='',trigger_price='',algo_price='',callback_rate='',
                   algo_variance='',avg_amount='',limit_price='',sweep_range='',sweep_ratio='',single_limit='',time_interval=''):

        if order_type==1:
            # 止盈止损
            params = {"instrument_id":instrument_id,"mode":mode,"order_type":order_type,"size":size,'side':side,"trigger_price":trigger_price,'algo_price':algo_price,}
        elif order_type==2:
            # 跟踪委托
            params = {"instrument_id":instrument_id,"mode":mode,"order_type":order_type,"size":size,'side':side,"trigger_price":trigger_price,'callback_rate':callback_rate,}
        elif order_type == 3:
        # 跟踪委托
    ...
    def cancel_batch_algos(self,instrument_id='',algo_ids='',order_type='',):
        ...
        return self._request_with_params(POST, SPOT_CANCEL_BATCH_ALGOS, params)
    def get_algo(self,
        instrument_id='',
        order_type='',
        status='',
        ...
        return self._request_with_params(GET, SPOT_ALGO, params)