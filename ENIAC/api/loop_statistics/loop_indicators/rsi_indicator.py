...
class iRsiLong(iBaseIndicator):
    ...
    def next(self):
        rsilong = set([self.data.close[i] > self.rsi_short[i] > self.rsi_long[i] for i in range(1-self.args[2],1)])
...
class iRsiShort(iBaseIndicator):
    ...
    def next(self):
        rsishort = set([self.data.close[i] < self.rsi_short[i] < self.rsi_long[i] for i in range(1-self.args[2],1)])
